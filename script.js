...
var turn = {
  ...
  resize: function () {
    // Reset width/height to CSS defaults
    this.el.style.width = "";
    this.el.style.height = "";

    // Get available space
    var maxWidth = this.el.clientWidth,
        maxHeight = Math.round(document.body.clientHeight * 0.9); // 90% of viewport height

    // Calculate dimensions:
    // - Each page should be square (width = height)
    // - Total book width = 2 × height (since two pages are displayed)
    var height = Math.min(maxHeight, maxWidth / 2);
    var width = height * 2; // Double width for two pages
